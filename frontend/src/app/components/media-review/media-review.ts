...
export interface MediaReviewSubmission {
  comment: Comment;
  rating: Rating;
}
...
export class MediaReview {
  private dialogRef = inject(MatDialogRef<MediaReview>);
  ...
  constructor() {
    if (this.isEditMode) {
      this.reviewForm.patchValue({
        score: this.data.rating!.score,
        commentText: this.data.comment!.commentText
      });
    }
  }
